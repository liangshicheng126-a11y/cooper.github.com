...
export default function ProjectDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Map ID to project data from translations
  ...
  // Define images based on ID (using placeholders for now)
  ...
  return (
    ...
    >
      ...
      <motion.div 
        variants={item}
        className="aspect-[21/9] rounded-[40px] overflow-hidden glass border-white/10 mb-16"
      >
        <div 
      ...
      <motion.div variants={item} className="grid grid-cols-3 gap-8 mb-24">
        ...
        <div className="glass p-8 rounded-3xl border-white/5">
          ...
          <p className="text-lg font-bold">{id === 'p1' ? t.portfolio.categories.graphic : id === 'p2' ? t.portfolio.categories.uiux : id === 'p3' ? t.portfolio.categories.photography : t.portfolio.categories.video}</p>
        </div>
      </motion.div>
