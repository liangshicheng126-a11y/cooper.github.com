...
- **Release resilience:** every GitHub Pages artifact includes the current and immediately previous Next.js static bundles. The export postbuild moves a critical inline guard to the first position in every document head, before styles and the Next.js bootstrap. It detects missing Next.js scripts, styles, and chunk imports, performs one cache-bypassing reload, and removes its temporary parameter as soon as the refreshed document starts; loop prevention remains active for failed recovery attempts.
