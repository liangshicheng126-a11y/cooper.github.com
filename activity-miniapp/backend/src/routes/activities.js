...
router.put('/:id/wx-group',        auth, c.patchWxGroupChat)
router.patch('/:id/wx-group',       auth, c.patchWxGroupChat)
router.get('/:id',                 softAuth, c.getById)
router.put('/:id',                 auth, validate(schemas.createActivity), c.update)
router.put('/:id/offline',         auth, c.offline)
router.get('/:id/sub-activities',  softAuth, c.getSubActivities)
