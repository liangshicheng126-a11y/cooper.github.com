...
- GitHub Pages releases retain the immediately previous `_next/static` bundle alongside the current bundle so cached or suspended mobile tabs can finish loading after a deployment. The export postbuild moves a pre-bootstrap inline asset guard to the first position in every document head; it performs at most one cache-bypassing refresh for a missing Next.js script or stylesheet, then removes its temporary query parameter as soon as the refreshed document starts. A root recovery surface offers explicit reload and retry actions for unrelated client exceptions.
